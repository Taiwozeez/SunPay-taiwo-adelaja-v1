...
import { HiChat, HiX, HiPhone, HiMail, HiOutlineLightBulb, HiOutlineCurrencyDollar, HiOutlineExclamation, HiOutlineCog, HiArrowLeft } from "react-icons/hi"
...
export default function FloatingChat() {
  ...
  const paymentIssues: Issue[] = [
    {
      ...
      solution: "1. Verify account balance\n2. Check network connection\n3. Contact your bank\n4. Try alternative payment"
    },
    {
      id: "payment-2",
      icon: <HiOutlineExclamation className="w-5 h-5" />,
  ...
  const contactInfo: ContactInfo[] = [
    {
      ...
      action: () => { window.location.href = 'tel:+2348001234567' }
    },
    {
      icon: <HiMail className="w-4 h-4" />,
      label: "Email Support",
      value: "support@sunpay.com",
      action: () => { window.location.href = 'mailto:support@sunpay.com' }
    }
  ]
  ...
  return (
    <>
          ...
          >
            ...
            <div className="h-96 overflow-y-auto p-4 space-y-4">
              {messages.map((msg) => (
                ...
                >
                  <div
                    className={`max-w-[90%] rounded-2xl px-4 py-3 ${msg.isBot
                      ? 'bg-gradient-to-r from-yellow-50 to-yellow-100 dark:from-yellow-900/20 dark:to-yellow-800/20 text-gray-800 dark:text-gray-200 border border-yellow-200 dark:border-yellow-800/30 rounded-tl-none'
              ...
              {viewingIssue && (
                <div className="mt-4">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-gradient-to-r from-yellow-500 to-yellow-600 text-white hover:from-yellow-600 hover:to-yellow-700 transition-all shadow-sm hover:shadow-md"
                    title="Return to menu"
                    aria-label="Return to menu"
                  >
                    <HiArrowLeft className="w-4 h-4" />
                    <span className="font-medium">Return to Menu</span>
                  </button>
                </div>
              )}
              ...
              {!viewingIssue && messages.length <= 1 && (
                <div className="mt-4">
                  <div className="flex items-center gap-2 mb-2">
                    <TbSolarPanel className="w-4 h-4 text-yellow-600 dark:text-yellow-400" />
                  ...
                  <div className="space-y-2">
                    {solarIssues.map((issue) => (
                      <button
                        ...
                        className="w-full text-left p-3 rounded-lg border border-yellow-200 dark:border-yellow-800/30 hover:bg-gradient-to-r from-yellow-50 to-yellow-100 dark:from-yellow-900/10 dark:to-yellow-800/10 transition-all group"
                        title={`Select ${issue.title}`}
                        aria-label={`Select ${issue.title}: ${issue.description}`}
                      >
                        <div className="flex items-center gap-3">
                          <div className="p-2 rounded-lg bg-gradient-to-r from-yellow-500 to-yellow-600 text-white">
                            {issue.icon}
                          </div>
                          <div>
                            <p className="font-medium text-sm text-gray-800 dark:text-gray-200 group-hover:text-yellow-600 dark:group-hover:text-yellow-400">
                              {issue.title}
                            </p>
              ...
              {!viewingIssue && messages.length <= 1 && (
                <div className="mt-4">
                  <div className="flex items-center gap-2 mb-2">
                    <HiOutlineCurrencyDollar className="w-4 h-4 text-yellow-600 dark:text-yellow-400" />
              ...
              {!viewingIssue && messages.length > 1 && (
                <>
                  <div className="mt-6">
                    <div className="bg-gradient-to-r from-yellow-50 to-yellow-100 dark:from-yellow-900/20 dark:to-yellow-800/20 rounded-lg p-4 mb-4">
                      <p className="text-sm font-medium text-yellow-700 dark:text-yellow-300 mb-2">Select another issue or contact support:</p>
                    </div>
                  </div>
            ...
            {/* Contact Info */}
            <div className="border-t border-yellow-200 dark:border-yellow-800/30 p-3 bg-gradient-to-r from-yellow-50 to-yellow-100 dark:from-yellow-900/10 dark:to-yellow-800/10">
              <p className="text-xs font-medium text-yellow-700 dark:text-yellow-300 mb-2">Need human assistance?</p>
              <div className="flex gap-2">
                {contactInfo.map((contact, index) => (
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-xs font-medium bg-gradient-to-r from-yellow-500 to-yellow-600 text-white hover:from-yellow-600 hover:to-yellow-700 rounded-lg transition-all shadow-sm hover:shadow-md"
                    title={contact.label}
      ...
      <motion.div
        className="fixed bottom-6 right-6 z-40 flex items-end space-x-2"
      ...
      >
        ...
        <AnimatePresence>
          {showMessage && (
            <motion.div
              className="bg-gradient-to-r from-yellow-500 to-yellow-600 text-white shadow-lg rounded-xl px-4 py-2 text-sm font-medium flex items-center gap-2"
            ...
            >
              ...
              <span>Need help with solar or payments?</span>
            </motion.div>
          )}
        </AnimatePresence>

        {/* Chat Icon */}
        <motion.div className="relative flex items-center justify-center" animate={floatAnimation}>
          {/* Radar ping effects - Yellow theme */}
          <motion.span
            className="absolute inline-flex h-16 w-16 rounded-full bg-gradient-to-r from-yellow-500 to-yellow-600 opacity-40"
            variants={pulseVariants}
            animate="pulse1"
          />
          <motion.span
            className="absolute inline-flex h-12 w-12 rounded-full bg-gradient-to-r from-yellow-500 to-yellow-600 opacity-50"
            variants={pulseVariants}
            animate="pulse2"
          />
          ...
          <motion.button
            type="button"
            className="relative z-10 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white rounded-full p-3 shadow-lg"
            ...
            animate={{
              ...
              backgroundColor: showChatBox ? "#b45309" : "#f59e0b"
            }}
            transition={{
              duration: 2,
              repeat: Infinity,
            ...
            title={showChatBox ? "Close chat" : "Open chat"}
            aria-label={showChatBox ? "Close chat assistant" : "Open chat assistant"}
          >
            {showChatBox ? (
